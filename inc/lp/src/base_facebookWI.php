 ...
 */
class FacebookApiExceptionWI extends Exception
{
  /**
...
abstract class BaseFacebookWI
{
  ...
  public function getSignedRequest() {
    ...
    if (!$this->signedRequest) {
      ...
      } else if (!empty($input_cookie[$this->getSignedRequestCookieName()])) {
        $this->signedRequest = $this->parseSignedRequest(
          $input_cookie[$this->getSignedRequestCookieName()]);
      }
    }
   ...
   * The cookie is not set by the BaseFacebookWI class, but
   ...
   * metadata. The cookie is not set by the BaseFacebookWI class, but it may be
  ...
  protected function getCode() {
    ...
    $server_info   = array_merge($input_get, $post_input, $input_cookie);
  ...
  protected function makeRequest($url, $params, $ch=null) {
    ...
    if ($result === false) {
      $e = new FacebookApiExceptionWI(array(
  ...
  protected function throwAPIException($result) {
    $e = new FacebookApiExceptionWI($result);
    switch ($e->getType()) {
      // OAuth 2.0 Draft 00 style
      case 'OAuthException':
        // OAuth 2.0 Draft 10 style
  ...
  public function destroySession() {
    ...
    if (array_key_exists($cookie_name, $input_cookie)) {
      unset($input_cookie[$cookie_name]);
      if (!headers_sent()) {
  ...
  protected function getMetadataCookie() {
    ...
    $cookie_value = trim($input_cookie[$cookie_name], '"');
  ...
  /**
   * Get the data for $key, persisted by BaseFacebookWI::setPersistentData()
   *
